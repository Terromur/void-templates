...
- WINE_USE_EGL - This is required for OpenGL to work on winewayland
...
- WINE_DISABLE_RAWINPUT - force disable rawinput.
- WINE_STATIC_CPUFREQ — Forces Wine to report a static CPU frequency instead of reading dynamic values from /sys. Set to any non-zero value. Useful for games that misbehave when CPU frequency fluctuates (e.g., timing or performance detection issues).
...
- SPA_PLUGIN_DIR - Path to custom libspa plugin.
- WINE_PIPEWIRE_QUANTUM - If winepipewire.drv is enabled, you can tune quantum here to adjust latency (default 128)
- WINE_PULSE_MEMLOCK - Try memlocking the audio buffer for winepulse.
